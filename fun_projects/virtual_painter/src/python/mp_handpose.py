...
class MPHandPose:
    ...
    def _preprocess(self, image, palm):
        ...
        Parameters:
          ...
          + - palm bounding box found in image of format [[x1, y1], [x2, y2]] (top-left and bottom-right points)
        ...
        ifmap, palm_bbox, bias = self._cropAndPadFromPalm(image, palm_bbox, True)
        ifmap = cv.cvtColor(ifmap, cv.COLOR_BGR2RGB)
        pad_bias += bias

        # Rotate input to have vertically oriented hand ifmap
        # compute rotation
        palm_bbox -= pad_bias
        ...
        #  get rotated ifmap
        rotated_ifmap = cv.warpAffine(ifmap, rotation_matrix, (ifmap.shape[1], ifmap.shape[0]))
        ...
        crop, rotated_palm_bbox, _ = self._cropAndPadFromPalm(rotated_ifmap, rotated_palm_bbox)
    ...
    def _postprocess(self, blob, rotated_palm_bbox, angle, rotation_matrix, pad_bias):
        ...
        # logistic fn for confidence and handedness
        conf = 1 / (1 + np.exp(-conf))
        handedness = 1 / (1 + np.exp(-handedness))
        landmarks = blob[2]
        landmarks_word = blob[3]
        ...
        return {'bbox':bbox.reshape(-1), 'landmarks': landmarks.reshape(-1), 'rotated_landmarks_world': rotated_landmarks_world.reshape(-1), 'handedness': handtype, 'conf': conf}