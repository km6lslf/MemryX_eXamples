...
class MPPalmDet:
    def __init__(self, nmsThreshold=0.3, scoreThreshold=0.5, topK=5000):
    ...
    def _preprocess(self, image):
        ...
        if image.shape[0] != self.input_size[0] or image.shape[1] != self.input_size[1]:
            ...
            ifmap = cv.resize(image, (ratio_size[1], ratio_size[0]))
            ...
            right = pad_w - left
            bottom = pad_h - top
            ifmap = cv.copyMakeBorder(ifmap, top, bottom, left, right, cv.BORDER_CONSTANT, None, (0, 0, 0))
        ifmap = cv.cvtColor(ifmap, cv.COLOR_BGR2RGB)    
        ifmap = ifmap.astype(np.float32) / 255.0        # norm
        pad_bias = (pad_bias / ratio).astype(np.int32)

        return ifmap[np.newaxis, :, :, :], pad_bias     # hwc -> nhwc

    def _postprocess(self, output_blob, original_shape, pad_bias):

        score = output_blob[0][0, :, 0]
        box_delta = output_blob[1][0, :, 0:4]
        landmark_delta = output_blob[1][0, :, 4:]
